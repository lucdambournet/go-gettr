...
interface BillItem { id: number; delay: number; angle: number; distance: number; }

// Animated bill that flies out during payout dump
function FlyingBill({ delay, angle, distance, onDone }: { delay: number; angle: number; distance: number; onDone: () => void }) {
  return (
    <motion.div
...
export default function CashPipe({ person, personIndex, fillPercent, amount, name, paying, onPayAnimDone }: CashPipeProps) {
  const PIPE_H = 280;
