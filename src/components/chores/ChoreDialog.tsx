...
export interface ChoreIconDef { key: string; label: string; Icon: ElementType; }
...
export function getChoreIcon(key: string | undefined): ElementType {
...
export default function ChoreDialog({ open, onOpenChange, chore, people, onSave, isSaving }: ChoreDialogProps) {
  const isEdit = !!chore;
  ...
  const [payoutPerCompletion, setPayoutPerCompletion] = useState<number | string>(0);
