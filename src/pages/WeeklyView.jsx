...
function WeekNav({ weekStart, weekDir, onPrev, onNext }) {
...
function ChoreRow({ chore, weekDays, logMap, optimistic, onToggle, personId }) {
  ...
  // Merge server logMap with optimistic overrides (null = removed)
  const effectiveLogMap = { ...logMap };
...
function PersonSection({ person, personIndex, chores, weekDays, logMap, optimistic, onToggle, weekStartStr }) {
          ...
          <CardHeader className="pb-3 pt-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <AnimatePresence mode="wait">
                  {allComplete && person.weekly_allowance > 0 ? (
...
export default function WeeklyView() {
  ...
  const [optimistic, setOptimistic] = useState({}); // key -> true/false (checked or null=deleted)
  ...
  const createLogMutation = useMutation({
    mutationFn: (data) => base44.entities.ChoreLog.create(data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["choreLogs"] }),
  });
  const deleteLogMutation = useMutation({
    mutationFn: (id) => base44.entities.ChoreLog.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["choreLogs"] }),
  });
  const updateStreakMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.Streak.update(id, data),
  ...
  const logMap = useMemo(() => {
    const map = {};
    logs.forEach((log) => { if (log.completed) map[`${log.chore_id}_${log.day}`] = log; });
    return map;
  }, [logs]);

  // Derive effective map: optimistic true = checked, null = unchecked
  const effectiveOptimistic = useMemo(() => {
    const o = {};
    Object.entries(optimistic).forEach(([k, v]) => { if (v === true) o[k] = true; });
    // For null entries, we need to shadow the logMap — handled in ChoreRow/PersonSection
    return optimistic;
  }, [optimistic]);

  const handleToggle = (choreId, personId, dayStr) => {
    ...
    if (existing) {
      ...
      // Deduct payout from bank
      if (chore?.payout_per_completion > 0) {
        const streak = streakMap[personId];
        if (streak) {
          updateStreakMutation.mutate({ id: streak.id, data: { total_rewards_earned: Math.max(0, (streak.total_rewards_earned || 0) - chore.payout_per_completion) } });
        }
      }
    } else {
      ...
      if (chore?.payout_per_completion > 0) {
        const streak = streakMap[personId];
        if (streak) {
          updateStreakMutation.mutate({ id: streak.id, data: { total_rewards_earned: (streak.total_rewards_earned || 0) + chore.payout_per_completion } });
        } else {
          createStreakMutation.mutate({ person_id: personId, current_streak: 0, longest_streak: 0, last_checkin_date: null, total_rewards_earned: chore.payout_per_completion });
        }
      }
    }
  };
  ...
  // Effective logMap that merges server data + optimistic overrides
  const effectiveLogMap = useMemo(() => {
    const m = { ...logMap };
    Object.entries(optimistic).forEach(([k, v]) => {
      if (v === true) m[k] = true;
  ...
  // Overall week stats for summary bar — uses effectiveLogMap so it syncs with toggles
  ...
  return (
    <div>
      ...
      {/* Main layout: content + right progress bar */}
      <div className="flex gap-5">
        {/* Left: person sections */}
        <div className="flex-1 min-w-0">
        ...
        {/* Right: vertical week progress bar */}
