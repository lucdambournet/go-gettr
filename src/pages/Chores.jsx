...
function getFreqDisplay(frequency) {
  if (!frequency) return FREQ_LABELS.weekly;
  if (FREQ_LABELS[frequency]) return FREQ_LABELS[frequency];
...
function ChoreCard({ chore, onEdit, onDelete, isDeleting }) {
            ...
            <div className="flex flex-wrap gap-1.5 items-center">
              ...
              {hasPayout && (
                <Badge className="text-[10px] px-2 py-0 h-5 border bg-emerald-500/10 text-emerald-600 border-emerald-400/30 font-bold">
                  <DollarSign className="w-2.5 h-2.5" />{chore.payout_per_completion.toFixed(2)} each
                </Badge>
              )}
            </div>
          </div>
...
export default function Chores() {
  ...
  const { data: chores = [], isLoading } = useQuery({ queryKey: ["chores"], queryFn: () => base44.entities.Chore.list() });
  ...
  return (
    <div className="space-y-8">
      ...
      <ChoreDialog open={!!editChore} onOpenChange={(v) => !v && setEditChore(null)} chore={editChore} people={activePeople}
        onSave={(data) => updateMutation.mutate({ id: editChore.id, data })} isSaving={updateMutation.isPending} />
        ...
        {activeChores.length === 0 ? (
          ...
          <motion.div key="content" className="space-y-10">
            {byPerson.map(({ person, chores: pChores }, gi) => {
              ...
              const paidChores = pChores.filter(c => c.payout_per_completion > 0);
