...
export default function DevPanel() {
  ...
  const { data: choreLogs = [] } = useQuery({ queryKey: ["allChoreLogs"], queryFn: () => base44.entities.ChoreLog.list() });
  const { data: payouts = [] } = useQuery({ queryKey: ["payouts"], queryFn: () => base44.entities.Payout.list() });
  ...
  const handleAddAllowances = async () => {
    ...
    try {
      for (const person of kids) {
        ...
        const freshStreaks = await base44.entities.Streak.list();
