...
import { Palette, RotateCcw, Check, Sparkles, Plus, Trash2, X, Bell, DollarSign, Globe, Moon, Sun, Monitor } from "lucide-react";
...
// Derive safe secondary/muted/hover from background lightness
function deriveTokens(vars) {
  const bg = vars.background || "225 20% 97%";
  const parts = bg.split(" ").map(parseFloat);
  const [h, s, l] = parts;
  const isDark = l < 30;

  // secondary: slightly darker/lighter than bg for hover safety
...
function ColorSwatch({ label, desc, hexVal, onChange }) {
...
function ThemeChip({ theme, isActive, onClick, onDelete, isCustom }) {
  return (
...
export default function Settings() {
  const [current, setCurrent] = useState(() => {
    try { return JSON.parse(localStorage.getItem(STORAGE_KEY)) || DEFAULT_THEME; } catch { return DEFAULT_THEME; }
  });
  ...
  const handlePreset = (theme) => { setActiveName(theme.name); setCurrent(theme.vars); applyTheme(theme.vars); };
  const updateVar = (key, hexVal) => {
    const hsl = hexToHsl(hexVal);
    const next = { ...current, [key]: hsl };
  ...
  const handleSaveAsNew = () => {
    ...
    const t = { name: newThemeName.trim(), dot1: hslToHex(current.primary), dot2: hslToHex(current.accent), dot3: hslToHex(current.background), vars: { ...current }, isCustom: true };
