...
import { CheckCircle2, Circle, Sun, Sparkles, DollarSign, Trophy, Flame, Star } from "lucide-react";
...
function getExpectedCompletions(chore) {
  const freq = chore.frequency || "weekly";
  if (freq === "twice_daily" || freq.startsWith("2x:")) return 2;
  return 1;
}

// Confetti particle on completion
function CompletionBurst({ x, y }) {
...
function ChoreCard({ chore, isCompleted, onToggle, personName, personColor, personAvatarColor, index }) {
  const ChoreIcon = getChoreIcon(chore.icon);
  const hasPayout = chore.payout_per_completion > 0;
  ...
  const handleToggle = (e) => {
    if (!isCompleted) {
      soundComplete();
      // Get card position for stamp animation
      if (cardRef.current) {
        ...
        <div className="p-4 pl-5">
          <div className="flex items-start gap-3">
            ...
            <motion.div
              ...
              style={isTwiceDaily && !isCompleted ? { ringColor: "#f59e0b", boxShadow: "0 0 0 2px #f59e0b88, 0 0 0 3px #f59e0b22" } : {}}
            >
                ...
                {hasPayout && (
                  ...
                  >
                    <Badge className="bg-emerald-500/10 text-emerald-600 border-0 text-[10px] font-bold gap-0.5">
                      <DollarSign className="w-2.5 h-2.5" />{chore.payout_per_completion.toFixed(2)}
                    </Badge>
                  </motion.div>
                )}
              </div>
              ...
              >
                <motion.div whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.97 }}>
                  <Button size="sm" className="w-full gap-2 h-9 rounded-xl font-semibold" onClick={handleToggle}>
                    ...
                    Mark Complete{hasPayout ? ` · +$${chore.payout_per_completion.toFixed(2)}` : ""}
                  </Button>
                </motion.div>
              </motion.div>
            )}
...
function PersonSection({ person, personIndex, chores, logMap, onToggle, weekStartStr, todayStr }) {
...
export default function Daily() {
  ...
  const handleToggle = async (chore, personId, dayStr) => {
    ...
    if (existing) {
      ...
      // Deduct payout from bank if applicable
  ...
  const todayStats = useMemo(() => {
    ...
    activePeople.forEach((person) => {
      const pChores = activeChores.filter((c) => c.assigned_to === person.id);
      const tChores = getTodayChores(pChores, today);
      total += tChores.length;
      done += tChores.filter((c) => logMap[`${c.id}_${todayStr}`]).length;
    });
